...
/**
 * Generates distinct random (x, y) pairs in range 0..8 EXACTLY as provided in
 * Lab 10 handout.
 */
public class RandomPairs {

    private static final int MAX_COORD = 8;
    private static final int MAX_UNIQUE_PAIRS
    ...
    /**
     * Generate n distinct random pairs (x, y) where 0 <= x,y <= 8
     */
    public List<int[]> generateDistinctPairs(int n) {
        if (n < 0 || n > MAX_UNIQUE_PAIRS) {
            throw new IllegalArgumentException(
                    "n must be between 0 and " + MAX_UNIQUE_PAIRS
            );
        }
